	...
	convertCurrency() {
		...
		return new Promise(resolve => {
			request(currencyApi, { json: true }, (err, res, body) => {
				...
				this.subtotal = (this.subtotal * this.rate / 100).toFixed(2);
				this.discountAmt = (this.discountAmt * this.rate / 100).toFixed(2);
				this.total = (this.total * this.rate / 100).toFixed(2);
				resolve(this);
			});
		});
	}
